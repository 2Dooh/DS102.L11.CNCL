...
class ResidualGenomeDecoder(ChannelBasedDecoder):
    ...
    def __init__(self, 
        ...
        phases = []
        for idx, (gene, (in_channels, out_channels), kernel_size) in enumerate(zip(self._genome, self._channels, self._kernel_sizes)):
            phases.append(ResidualPhase(gene, in_channels, out_channels, kernel_size, idx, preact=preact))
...
class VariableGenomeDecoder(ChannelBasedDecoder):
    ...
    def __init__(self, 
        ...
        genome_copy = list_genome.copy()  # We can't guarantee the genome won't be changed in the parent constructor.

        super().__init__(list_genome, channels, kernel_sizes, pool_sizes, repeats=repeats)
        ...
        phases = []
        for idx, (gene, (in_channels, out_channels), kernel_size, phase_type) in enumerate(zip(self._genome, 
                                                                                               self._channels, 
                                                                                               self._kernel_sizes, 
                                                                                               self._types)):
            if phase_type == self.RESIDUAL or phase_type == 3:
                phases.append(ResidualPhase(gene, in_channels, out_channels, kernel_size, idx))

            elif phase_type == self.PREACT_RESIDUAL:
                phases.append(ResidualPhase(gene, in_channels, out_channels, kernel_size, idx, preact=True))

            elif phase_type == self.DENSE:
                phases.append(DensePhase(gene, in_channels, out_channels, kernel_size, idx))
    ...
    def __init__(self, gene, in_channels, out_channels, kernel_size, idx):
        ...
        for i in range(len(gene)):
            if len(self.dependency_graph[i + 1]) > 0:
                ...
                nodes.append(DenseNode(channels, kernel_size))
